	...
	this.getEditorControl = function(){
		if (this.idRelatedEntity > 0) {
			...
			if ( isNumeric(this.dataType) || isMoney(this.dataType) ) {
				...
				getDescendants(this.editor, 'booleanEditor')[0].onkeydown = this.internalEndEditFunction;
				getDescendants(this.editor, 'booleanEditor')[1].onkeydown = this.internalEndEditFunction;
	...
	this.getEditorValue = function(){
		if (this.idRelatedEntity > 0)	{
			...
			if (getDescendants(this.editor, 'booleanEditor')[0].checked) {
				return window.parent.bizagi.localization.getResource("workportal-general-word-true");
			} else {
				return window.parent.bizagi.localization.getResource("workportal-general-word-false");
			}
		} else { // Default text editor
