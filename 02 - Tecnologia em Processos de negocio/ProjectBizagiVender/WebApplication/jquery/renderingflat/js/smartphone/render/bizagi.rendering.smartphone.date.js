...
bizagi.rendering.date.extend("bizagi.rendering.date", {
    ...
    getFormattedDate: function(date, showTime, inicializer) {
        ...
        var time = (showTime && (self.getTime() === "0:0" || inicializer)) ? " " + 
		date.substr(date.length - 8, date.length) : "";
        date = bizagi.util.dateFormatter.getDateFromInvariant(((inicializer && showTime && time.split("/").length > 1) || (date.split(":").length === 1)) ? 
    ...
    onChangeHandler: function(isClearButton) {
        ...
        if ((currentDate === "" && !properties.showTime) || (currentDate === "" && currentTime === "") || (isClearButton)) {
            ...
            if (!formatedFullDate || formatedFullDate == 0) {
                return;
            }
            ...
            displayValue = self.preprocessDateValue(formatedFullDate);
    ...
    actionSave: function() {
        ...
        if (bizagi.util.isIphoneHigherIOS5()) {
            ...
            self.input.html(bizagi.util.dateFormatter.formatDate(date, properties.fullFormat));
