...
//TODO	tmplFnsCache = {},
    ...
    function fastRender(data, context, noIteration, parentView, key, onRender) {
        ...
        if (!parentView && self.fn._nvw && !$.isArray(data)) {
            content = tryFn(self, data, {tmpl: self});
        } else {
            content = renderContent.call(self, data, context, noIteration, parentView, key, onRender);
        }

        content = $(content);
        // Execute custom stuff after the template has been processed
        processLoading.call( self, content);
        processAdjustable.call( self, content);
    ...
    */
    function processLoading(content){
        if ($("[data-loading=true]", content).length > 0) {
            $("[data-loading=true]", content).loadingMessage();
        }
        $("[data-loading=true]", content).addClass("ui-bizagi-component-loading");
        if (content.jquery) {
    ...
    *  Configure adjustable containers
     */
    function processAdjustable(content){

        $("[data-adjustable=true]", content).addClass("ui-bizagi-component-adjustable");
