/*
*   @title : Association Combo Wizard
* @author : Alexander Mejia
* @date   : 05/March/2014 
* Comments:
*     Define the association combo wizard
*/

bizagi.editor.wizard.base("bizagi.editor.wizard.association", {
    ...
    */
    init: function (canvas, params) {
        ...
        this.model = new bizagi.editor.wizard.association.model(params.model);
    },
    ...
    */
    loadTemplates: function () {
        // Define mapping
        var templateMap = {
            "wizard-container": (bizagi.getTemplate("bizagi.editor.wizard.association") + "#wizard-container-association")
        };
    ...
    */
    processFinishButton: function () {
        ...
        if (!self.model.isValid()) {
            bizagi.showMessageBox(bizagi.localization.getResource("bizagi-editor-wizard-association-invalidmodel"), "Bizagi", "error", false);
            return;
        }
        ...
        $.when(self.relationshipIsValid())
            .done(function (data) {
                if (data.isValid) {
                    ...
                    bizagi.showMessageBox(bizagi.localization.getResource("bizagi-editor-wizard-association-invalidrelationship"), "Bizagi", "error", false);
                    return;
                }
        ...
        $.when(self.presenter.publish("getMultipleRelationship", {
            ...
            .done(function (data) {
                if (!data) {
                    ...
                    var rightfacts = data.rightfact;

                    for (var i = 0, l = leftfacts.length; i < l; i++) {
                        var leftfact = leftfacts[i];
                        if ((leftfact.LeftEntity.Guid == leftRelatedEntity) && (leftfact.RightEntity.Guid == rightRelatedEntity)) {
                            defer.resolve({
                                isValid: true,
                                factxpath: leftfact.Name,
                                leftfactname: leftfact.Name,
                                rightfactname: rightfacts[0] && rightfacts[0].Name
                            });
                            break;
                        }
                    }
    ...
    *****************************************************************************************************/
    ...
    ".bizagi_editor_component_editor_xpath .bz-wizard-association-editor-xpath click": function (element) {
        var self = this;
    ...
    */
    ".biz-ico-filter-value click": function (element) {
        var self = this;
        ...
        params.data = propertyValue ? propertyValue : { bafilter: { filter: "filter"} };
        params.entity = self.resolveComplexXpath(self.getRelatedEntity(xpathProperty));
