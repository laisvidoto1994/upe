...
bizagi=bizagi||{},function(n){bizagi.rateUs=function(){var t,o,i;if(bizagi.override.enableRateUS&&bizagi.util.isCordovaSupported()&&bizagi.detectSO()==="android"){t=null;o=!1;try{t=JSON.parse(bizagi.util.getItemLocalStorage("BizagiLogins"))}catch(nt){}if(t=t||{},t.every=20,t.minLapsedTime=2592e6,t.nextRateIn=Math.max(t.nextRateIn,1),t.rate={rated:t.rate&&t.rate.rated,version:t.rate&&t.rate.version?t.rate.version:null,time:t.rate&&t.rate.time?t.rate.time:(new Date).getTime()},t.actualVersion!==bizagi.version.client.actual()?(t.actualVersion=bizagi.version.client.actual(),t.nextRateIn=t.every):t.nextRateIn--,bizagi.util.setItemLocalStorage("BizagiLogins",JSON.stringify(t)),i=function(n){var t=JSON.parse(bizagi.util.getItemLocalStorage("BizagiLogins")),i;switch(n){case"rate":window.open("market://details?com.bizagi.smartphone");t.rate={rated:!0,version:bizagi.version.client.actual(),time:(new Date).getTime()};break;case"never":t.rate={rated:!1,version:bizagi.version.client.actual(),time:(new Date).getTime()};break;case"later":i=(new Date).getTime()+6048e5-t.minLapsedTime;t.rate.time=new Date(i).getTime();t.nextRateIn=1e4;break;default:return}bizagi.util.setItemLocalStorage("BizagiLogins",JSON.stringify(t))},t.rate.version!==t.actualVersion&&(t.nextRateIn===0||t.rate.time&&(new Date).getTime()>new Date(t.rate.time).getTime()+t.minLapsedTime))if(o)navigator&&navigator.notification&&navigator.notification.confirm&&navigator.notification.confirm(bizagi.localization.getResource("bizagi-rateus-title","Like using Bizagi?"),function(n){n===1?i("rate"):n===2?i("later"):n===3&&i("never")},bizagi.localization.getResource("bizagi-rateus-description","Please take a minute to rate us!"),[bizagi.localization.getResource("bizagi-rateus-yes","YES"),bizagi.localization.getResource("bizagi-rateus-later","REMIND ME LATER"),bizagi.localization.getResource("bizagi-rateus-no","NO")]);else{var u=n(document.body),l={overflow:u.css("overflow"),height:"",width:""},f=n('<div class="bizagiRateUS_blackout"><\/div>').click(function(n){n.stopPropagation()}),a=function(){f.remove();u.css(l)},r=function(n,t){n.stopPropagation();n.preventDefault();i(t);a()},e=n('<div class="bizagiRateUS_content"><\/div>').click(function(n){n.stopPropagation()}),s=n('<div class="bizagiRateUS_content_body"><\/div>'),v=n('<img class="bizagiRateUS_logo" src="'+bizagiConfig.proxyPrefix+'jquery/common/bizagi.rateUs/img/bizagi-logo.svg" />'),y=n('<div class="bizagiRateUS_close"><i class="km-icon km-cancel km-notext"><\/i><\/div>').click(function(n){r(n,"later")}),p=n('<img class="bizagiRateUS_content_image"/>').prop("src",bizagiConfig.proxyPrefix+"jquery/common/bizagi.rateUs/img/stars.png"),w=n("<h1><\/h1>").text(bizagi.localization.getResource("bizagi-rateus-title","Like using Bizagi?")),b=n("<p><\/p>").text(bizagi.localization.getResource("bizagi-rateus-description","Please take a minute to rate us!")),h=n('<div class="bizagiRateUS_options"><\/div>'),c=n('<div class="bizagiRateUS_options_horizontal"><\/div>'),k=n('<div class="ui-bizagi-render-button new secondary">'+bizagi.localization.getResource("bizagi-rateus-no","NO")+"<\/div>").click(function(t){n(this).addClass("loading");r(t,"never")}),d=n('<div class="ui-bizagi-render-button new secondary">'+bizagi.localization.getResource("bizagi-rateus-later","REMIND ME LATER")+"<\/div>").click(function(t){n(this).addClass("loading");r(t,"later")}),g=n('<div class="ui-bizagi-render-button new primary">'+bizagi.localization.getResource("bizagi-rateus-yes","YES")+"<\/div>").click(function(t){n(this).addClass("loading");r(t,"rate")});s.append(p).append(w).append(b).append(y).append(v);e.append(s);c.append(k).append(d);h.append(c).append(g);e.append(h);f.append(e);u.css({overflow:"hidden",height:"100%",width:"100%"});n(window.document.body).append(f)}}}}(jQuery);
