	...
	return this.each( function() {
		...
		setColWidth = function () {
			...
			if(ts.p.shrinkToFit===true && vc > 0) {
                ...
                if (ts.p.grouping == false) {
					var lastColumn;
					$.each(ts.p.colModel, function(i) {
						if (this.hidden == false) lastColumn = this;
					});
					lastColumn.width = 0;
				}
			}
		...
		$("th",thr).each(function ( j ) {
			...
			if (w > 0) $(this).css("width",w+"px").prepend(res);
			else $(this).css("width","auto").prepend(res);
			var hdcol = "";
			if( ts.p.colModel[j].hidden ) {
				$(this).css("display","none");
				hdcol = "display:none;";
			}
			if (w > 0) firstr += "<td role='gridcell' style='height:0px;width:"+w+"px;"+hdcol+"'></td>";
			else firstr += "<td role='gridcell' style='height:0px;width:auto;"+hdcol+"'></td>";
			...
			if(ts.p.hidegrid===true) {
				$(".ui-jqgrid-titlebar-close",grid.cDiv).click( function(e){
					...
					if(ts.p.gridstate == 'visible') {
						$(elems, "#gbox_" + $.jgrid.jqID(ts.p.id)).hide();
						...
						$(elems, "#gbox_" + $.jgrid.jqID(ts.p.id)).show();
				...
				$.each($t.p.colModel, function(i) {
					if(this.hidden === false && !this.fixed){
						...
						if(this.width > 0){
							...
							$t.grid.headers[i].el.style.width="auto";
						}
						...
						}else if($t.p.footerrow){
							$t.grid.footers[i].style.width = "auto"; 
						}
						...
						if (this.width > 0){ 
							...
							$t.grid.cols[i].style.width = "auto"; 
						}
