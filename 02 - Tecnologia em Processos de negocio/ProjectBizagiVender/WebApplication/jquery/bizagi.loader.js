...
bizagi.replaceAll = function (text, pcFrom, pcTo) {
    var result = [];
    var zero = 0;
    var index = text.indexOf(pcFrom);
    if (index !== -1) {
        var arrText = text.split("");
        while (index !== -1) {
            // se almacena el texto previo a la coincidencia
            result = result.concat(arrText.slice(zero, index));
            // se obtiene el texto justo donde empieza la coincidencia
            arrText = arrText.slice(index);
            // se reemplaza el texto de la coincidencia
            arrText = arrText.join("").replace(pcFrom, pcTo).split("");
            // se almacena la coincidencia
            result = result.concat(arrText.slice(zero, pcTo.length));
            // se obtiene el texto justo donde termina la coincidencia
            arrText = arrText.slice(pcTo.length);
            // se actualiza el indice
            index = arrText.join("").indexOf(pcFrom);
        }
        result = result.concat(arrText);
        return result.join("");
    }
...
bizagi.isFlatDesignEnabled = function() {
    // return typeof (BIZAGI_ENABLE_FLAT) !== "undefined" && eval(BIZAGI_ENABLE_FLAT);
...
bizagi.detectDevice = function() {
    ...
    if (/Android|webOS|iPhone|iPod|iPad|BlackBerry/i.test(navigator.userAgent)) {
        ...
        if (navigator.userAgent.indexOf("iPad") >= 0) {
            //var enableFlatDesign = (typeof (BIZAGI_ENABLE_FLAT) != 'undefined' && eval(BIZAGI_ENABLE_FLAT));
            BIZAGI_DEFAULT_DEVICE = bizagi.isFlatDesignEnabled() ? "tablet_ios" : "tablet";
...
bizagi.isMobile = function() {
    ...
    if (bizagi.isMobileDevice() || typeof Windows != "undefined") {
...
bizagi.detectRealDevice = function() {
    var device;
    var settedDevice = (typeof (BIZAGI_DEFAULT_DEVICE) !== "undefined"
        && BIZAGI_DEFAULT_DEVICE !== "" ? BIZAGI_DEFAULT_DEVICE : null);
    ...
    if (settedDevice.indexOf("native") >= 0) {
        // device = settedDevice === "smartphone_ios_native" ? settedDevice : device + "_native";
...
bizagi.loaderPrototype = function () {
    ...
    this.init = function (params) {
        ...
        if (params.skipVersionCheck) {
            ...
            this.nativeAjax(versionUrl, function (data) {
                ...
                self.channel = JSON.parse(data.responseText).channel || "";
            });
        }
    ...
    this.loadModule = function (module, device) {
        ...
        if (module.devices) {
            var device = device || bizagi.detectDevice();
    ...
    this.loadReleaseModule = function (module) {
        ...
        if (module.components.required && module.components.required.length) {
            for (var i = 0; i < module.components.required.length; i++) {
                this.start(module.components.required[i]);
            }
        }
    ...
    this.loadDebugModule = function (module) {
        ...
        this.loadRequiredModules(module.components.required);
    ...
    this.loadRequiredModules = function (modules) {
    ...
    this.start = function (module, device) {
        ...
        if (modulesToShowLoading.indexOf(module) > -1 && typeof $ != "undefined" && typeof bizagi.loadModuleStatus != "undefined") {
            ...
            bizagi.templateService.getTemplate(bizagi.getTemplate("bizagi.workportal.desktop.loadModuleStatus"))
    ...
    this.getPathUrl = function (url) {
        ...
        var pathToBaseBackwards = BIZAGI_PATH_TO_BASE.split("../").length;
        for (i = 0; i < locationParts.length - pathToBaseBackwards; i++) {
...
bizagi.getWebpart = function (src) {
    ...
    if (webpart == null) {
        ...
        bizagi.cookie("bizagiDefaultWidget", "");
