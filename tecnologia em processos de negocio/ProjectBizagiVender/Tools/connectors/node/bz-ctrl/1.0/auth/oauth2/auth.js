...
function invoke(authData, systemProperties, restUrl, httpOperation, LOG, callback){		    
    ...
    if (grant_type === 'client_credentials'){
        clientCredentialsFlow(authData, systemProperties, restUrl, LOG, callback);
    }
    else if (grant_type === 'password'){
        resourceOwnerPasswordCredentialsFlow(authData, systemProperties, restUrl, LOG, callback);
    }    
    else{
...
function clientCredentialsFlow(authData, systemProperties, restUrl, LOG, callback){
    ...
    oauth2.clientCredentialsFlow(token_url, client_id, client_secret, scope, systemProperties, null, function (error, accessTokenResponse, statusCode, access_token, token_type, expires_in){		    
        if (error){
...
function resourceOwnerPasswordCredentialsFlow(authData, systemProperties, restUrl, LOG, callback){
    var error = bizagiUtil.validator.requiredParameters({'authdata.token_url': authData.token_url, 
    ...
    oauth2.resourceOwnerPasswordCredentialsFlow(token_url, username, password, scope, systemProperties, null, function (error, accessTokenResponse, statusCode, access_token, token_type, expires_in){		    
        if (error){
            var error = error.error || error;;
            if (error.error_description){
                error = error.error_description;
