...
bizagi.workportal.services.db.extend("bizagi.workportal.services.db", {}, {
    ...
    */
    ...
    saveFormInfo: function (params, data) {
        var self = this;
        var def = new $.Deferred();
        self.dbCases.get(bizagi.context.idCase, function (err, doc) {
            if (!err) {
                    ...
                    if (element.container) {
                        ...
                        if (element.render.properties.xpath == datafill.key) {
                            ...
                            if (!bizagi.util.isEmpty(response[datafill.key])) {
                                for (var row = 0, lenghtRows = response[datafill.key].value.length; row < lenghtRows; row++) {
                                    if (!bizagi.util.isEmpty(response[datafill.key].value[row])) {
                                        var tempDataColumn = [];
                                        for (var col = 0, lenghtCols = response[datafill.key].value[row].length; col < lenghtCols; col++) {
