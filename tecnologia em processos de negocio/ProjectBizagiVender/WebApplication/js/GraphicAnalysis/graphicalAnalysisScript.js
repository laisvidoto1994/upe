...
function saveResource(form, doSubmit) {
    saveResource(form, doSubmit, false);
}
...
function saveResource(form, doSubmit, bIsExcel) {
    ...
    for (var iDimension = 0; iDimension < selectedDimensions.length; iDimension++) {
        sDimensions += selectedDimensions[iDimension].columnName + ",";
        if (selectedDimensions[iDimension] && selectedDimensions[iDimension].filter && selectedDimensions[iDimension].filter.length > 0)
            aDimensionsFilters[aDimensionsFilters.length] = "h_dimensionfilter_" + selectedDimensions[iDimension].columnName + "=" + selectedDimensions[iDimension].filter;
    }
...
function renderContents(jsonData) {
    ...
    if (jsonData && jsonData.length > 0) {
        ...
        createChart();
        setTimeout("createPieCharts('Pie2D')", 500);
    }
    setTimeout("endProcessing()", 1000);
}
...
function resizeContainers(bOnLoad) {
    ...
    var width = windowSize[0] - findPos(xpTab1)[0] - 40;
    var height = windowSize[1] - findPos(xpTab1)[1] - 60;
...
function showSeparator() {
    showHideSeparatorOptions();
    startProcessing();
    var mainchartdata = document.getElementById("mainchartdivgrid");
    var chartSeparatorColumnsCombo = document.getElementById("h_chartseparatorcolumns");
    var chartSeparatorColumnsLabel = document.getElementById("chartseparatorcolumnslabel");
    var separatorDiv = document.getElementById("separatorDiv");
    ...
    if (bShowSeparator) {
        ...
        separatorDiv.style.visiblity = "visible";
        ...
        if (!secondaryTable) {
            secondaryTable = new BizAgiGridTable(null, mainchartdata, dataTable, mainTable.width, (mainTable.height / 2), false, true, true);
        }
        else {
            secondaryTable.dataTable = dataTable;
        }
        ...
        separatorDiv.style.visiblity = "hidden";
        separatorDiv.style.display = "none";

        m_ChartHeight = m_containerHeight - 10;
    }
    changeChartType();
    setTimeout("endProcessing()", 1000);
}
...
function populateAddMeasuresDialog() {
    ...
    for (var i = 0; i < selectedMeasures.length; i++) {
        tbody.appendChild(renderWizardMeasures(selectedMeasures[i].columnName, selectedMeasures[i].functions.join(',')));
    ...
    for (var i = 0; i < allMeasures.length; i++) {
        if (arrSelectedMeasures.join().indexOf(allMeasures[i].columnName) < 0) {
            tbody.appendChild(renderWizardMeasures(allMeasures[i].columnName));
        }
    }
}
...
function createDD(item, isChecked) {
    ...
    check.id = "check_" + item.columnName;
...
function showHideSeparatorOptions() {
    var div = document.getElementById("separatorDiv");
...
var griddiv = document.getElementById("listaMedidas");
debug.value = griddiv.outerHTML;
}
*/