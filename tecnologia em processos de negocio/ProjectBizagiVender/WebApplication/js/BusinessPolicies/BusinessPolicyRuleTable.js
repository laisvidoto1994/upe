...
function calculatePanelWidth(){
	...
	return document.getElementById(panelId).offsetWidth - panelDelta;
}
...
function resizePanels(){
	...
	if (BP_RENDERING_TYPE == 1){
		...
		document.getElementById('ruleTableDiv').style.width = iPanelWidth;
...
function paintPreview(ruleCode) {
	...
	// Paint the preview
	paintRule(ruleCode.getXml(), rulePreviewDiv);
}
...
function drawRuleTableView(paramRuleTable){
	...
	for(var i=0; i<policyRuleTable.rulesCode.length; i++){
		drawRuleCode(policyRuleTable.rulesCode[i]);
	}
...
function drawRuleCode(ruleCode){
	var cell;
	var iCount = 4;
	...
	containerDiv.innerHTML = ruleCodeTemplate.innerHTML;	
